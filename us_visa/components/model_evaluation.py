'''
Model Evaluation takes the trained model and evaluates it on the test data.
it returns the best model and the evaluation metrics.
Steps
1. Intially the model will be in local drive so we will check from S3 bucket for the availability of the model.
2. once model is pushed to s3 we compare the model in local with the model in S3 bucket with a threshold. 
then replace if the local model with s3 based on threshold.
